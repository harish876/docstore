...
public:
  ...
  // Overload Get mothod for returning the list of key,value pairs for query on
  // sec key

  // SECONDARY MEMTABLE
  bool Get(const LookupKey &key, std::string *value, Status *s, uint64_t *tag);
  void Get(const Slice &skey, SequenceNumber snapshot,
           std::vector<SecondayKeyReturnVal> *value, Status *s,
           std::unordered_set<std::string> *resultSetofKeysFound,
           int topKOutput);

  void RangeLookUp(const Slice &startSkey, const Slice &endSkey,
