  ...
  Rep(const Options &opt, WritableFile *f)
      : options(opt), index_block_options(opt), interval_block_options(opt),
        ...
        secondary_filter_block(opt.filter_policy == NULL
                                   ? NULL
                                   : new FilterBlockBuilder(opt.filter_policy)),
        maxSecValue(""), minSecValue(""), maxSecSeqNumber(0),
...
void TableBuilder::Add(const Slice &key, const Slice &value) {
  ...
  if (r->pending_index_entry) {
    ...
    // cout<<SSTR(fileNumber)+"+"+r->last_key.substr(0, r->last_key.size() - 8 )
    // <<" ,min: "<< rep_->minSecValue <<" ,max: "<< rep_->maxSecValue<<"
    // ,seqno: "<< rep_->maxSecSeqNumber<<std::endl;
